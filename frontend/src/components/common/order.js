...
function ViewOrders(props) {
    ...
    useEffect(() => {
        let ab = JSON.parse(localStorage.getItem('DASS_USEREMAIL'));
        let a = ab.email
        if (!a || a == 0)
            setID("")
        else
            setID(a)
        axios.get('http://localhost:4000/order/user/' + a)
            .then(response => {
                console.log(response.data)
                setOrder(response.data);
            })
        // also get the list of all the users
        axios.get('http://localhost:4000/user/register/')
            .then(response => {
                setAllUsers(response.data)
            })
        // also get the list of all the products
        axios.get('http://localhost:4000/product/vendor/')
            .then(response => {
